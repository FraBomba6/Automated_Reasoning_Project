...
void evaluate_model_and_write_results(FILE *jsonptr, char *file_path, int is_mzn) {
    ...
    while (fgets(line, 1000, dataptr) != NULL && line_count < 5) {
        ...
        if (strncmp(line, "n", 1) == 0 || strncmp(line, "#const n", 8) == 0) {
            ...
            fprintf(jsonptr, "%d", n);
            fprintf(jsonptr, "%s", ",\n");
        } else if (strncmp(line, "hearts", 6) == 0 || strncmp(line, "#const hearts", 13) == 0) {
            hearts = atoi(token);
            fprintf(jsonptr, "%s", "\t\t\t\t\"hearts\":");
            fprintf(jsonptr, "%d", hearts);
            fprintf(jsonptr, "%s", ",\n");
        } else if (strncmp(line, "clubs", 5) == 0 || strncmp(line, "#const clubs", 12) == 0) {
            clubs = atoi(token);
            fprintf(jsonptr, "%s", "\t\t\t\t\"clubs\":");
            fprintf(jsonptr, "%d", clubs);
            fprintf(jsonptr, "%s", ",\n");
        } else if (strncmp(line, "spades", 6) == 0 || strncmp(line, "#const spades", 13) == 0) {
            spades = atoi(token);
            fprintf(jsonptr, "%s", "\t\t\t\t\"spades\":");
            fprintf(jsonptr, "%d", spades);
            fprintf(jsonptr, "%s", ",\n");
        } else if (strncmp(line, "diamonds", 6) == 0 || strncmp(line, "#const diamonds", 13) == 0) {
            diamonds = atoi(token);
    ...
    char mzn_command[100] = "minizinc ./minizinc/config.mpc ./minizinc/model.mzn ";
