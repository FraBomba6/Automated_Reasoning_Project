...
cell *gen_cells(int n, int blocked, int occupied, int hearts, int clubs,
                int spades, int diamonds) {
  int x, y, hearts_counter = 0, clubs_counter = 0, spades_counter = 0,
            diamonds_counter = 0;
...
void gen_dzn(int n, int hearts, int clubs, int spades, int diamonds,
             cell *cells, int cells_l, char *mzn_dir, int m) {
  char file_name[15] = "";
  char file_number[4] = "";
  char file_path[50] = "";
...
void gen_lp(int n, int hearts, int clubs, int spades, int diamonds, cell *cells,
            int cells_l, char *asp_dir, int m) {
  char file_name[15] = "";
  char file_number[4] = "";
  char file_path[50] = "";
  sprintf(file_number, "%02d", m);
...
int main(int argc, char *argv[]) {
  ...
  srand(time(NULL));
  printf("Hi!\n"
         "This is the program to generate a set of base matrices needed as "
         "input for the Minizinc and ASP models.\n"
  ...
  for (int i = 1; i < argc || !cmd; i++) {
    ...
    for (int i = m; i > 0; i--) {
      ...
      cell *cells =
          gen_cells(n, blocked, occupied, hearts, clubs, spades, diamonds);
      printf("\t\t");
      for (int l = 0; l < blocked + occupied; l++) {
      ...
      gen_dzn(n, hearts, clubs, spades, diamonds, cells, blocked + occupied,
              mzn_dir, m - i + 1);
      gen_lp(n, hearts, clubs, spades, diamonds, cells, blocked + occupied,
             asp_dir, m - i + 1);
      printf("************************************************************\n");
    }
  }
